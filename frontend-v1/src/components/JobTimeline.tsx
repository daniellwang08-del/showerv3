...
export function JobTimeline({
  ...
  const onPostToSheet = useJobsStore((s) => s.postToSheet);
              ...
              <div key={dateKey} className="relative">
                      ...
                      <div className="relative">
                        ...
                        {bulkActionOpen === dateKey && (
                          ...
                          >
                            ...
                            <button
                              type="button"
                              className="block w-full border-b border-slate-100 px-3 py-2 text-left text-sm text-slate-700 hover:bg-emerald-50/80"
                              onClick={async () => {
                                const selectedJobs = getAllSelectedJobs();
                                setBulkActionOpen(null);
                                setSelectedJobsByDate({});
                                if (selectedJobs.length > 0) await onPostToSheet(selectedJobs);
                              }}
                            >
                              <span className="inline-flex items-center gap-2">
                                <Table2 className="h-4 w-4 shrink-0 text-emerald-600" aria-hidden />
                                Post to Sheet
                              </span>
                            </button>
                            <button
                ...
                <div className="ml-4 border-l-2 border-slate-200 pl-4 space-y-2">
                  {sortedGroupedByDate[dateKey].map((item) => {
                    ...
                    const isPostedToSheet = Boolean(item.sheet_posted_at);
                    return (
                    <div key={item.id} className="group">
                      <div className="relative">
                        <div 
                          ...
                          }${isPostedToSheet && !isApplied ? ' !border-l-[3px] !border-l-sky-500' : ''} ${
                            ...
                            const menuPanel = (
                              <>
                                ...
                                {item.table === 'valid' && (
                                  <button
                                    type="button"
                                    className="block w-full border-b border-blue-100 px-3 py-2 text-left text-sm text-slate-700 transition hover:bg-emerald-50"
                                    onClick={async () => {
                                      const targets = getContextMenuTargets(dateKey, item);
                                      closeMenu();
                                      clearSelectionIfMenuActionConsumedFullSelection(targets);
                                      await onPostToSheet(targets);
                                    }}
                                  >
                                    <span className="inline-flex items-center gap-2">
