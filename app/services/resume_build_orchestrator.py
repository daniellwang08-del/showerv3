...
async def run_resume_build(valid_job_id: str, user_id: str) -> dict | None:
    """
    ...
    try:
        async with get_session() as session:
            ...
            build = await repo.get(valid_job_id, user_id)
            if not build:
                logger.warning("resume_build_no_row", valid_job_id=valid_job_id)
                return None

            tailored = build.tailored_resume_data
            cover_data = build.cover_letter_data
            if not tailored:
                logger.warning("resume_build_no_tailored_data", valid_job_id=valid_job_id)
                return None
            ...
            company = (valid_job.company if valid_job else None) or "Unknown"
            position = (valid_job.title if valid_job else None) or "Unknown"
            ...
            try:
                ...
                await publish_resume_event({"type": "resume_file_processing", "user_id": user_id, "valid_job_id": valid_job_id, "file_type": "resume_docx"})
                ...
                await publish_resume_event({"type": "resume_file_ready", "user_id": user_id, "valid_job_id": valid_job_id, "file_type": "resume_docx"})
                ...
                await publish_resume_event({"type": "resume_file_failed", "user_id": user_id, "valid_job_id": valid_job_id, "file_type": "resume_docx", "error": str(e)})

            # --- Resume PDF ---
            if results.get("resume_docx"):
                try:
                    await repo.update_file_status(build.id, "resume_pdf", "processing")
                    await publish_resume_event({"type": "resume_file_processing", "user_id": user_id, "valid_job_id": valid_job_id, "file_type": "resume_pdf"})
                    ...
                    await publish_resume_event({"type": "resume_file_ready", "user_id": user_id, "valid_job_id": valid_job_id, "file_type": "resume_pdf"})
                except Exception as e:
                    ...
                    await publish_resume_event({"type": "resume_file_failed", "user_id": user_id, "valid_job_id": valid_job_id, "file_type": "resume_pdf", "error": str(e)})
            ...
            if cover_data and cover_data.get("body") and cl_template.exists():
                try:
                    ...
                    await publish_resume_event({"type": "resume_file_processing", "user_id": user_id, "valid_job_id": valid_job_id, "file_type": "cover_letter_docx"})
                    ...
                    await publish_resume_event({"type": "resume_file_ready", "user_id": user_id, "valid_job_id": valid_job_id, "file_type": "cover_letter_docx"})
                except Exception as e:
                    ...
                    await publish_resume_event({"type": "resume_file_failed", "user_id": user_id, "valid_job_id": valid_job_id, "file_type": "cover_letter_docx", "error": str(e)})

                # --- Cover Letter PDF ---
                if results.get("cover_letter_docx"):
                    try:
                        await repo.update_file_status(build.id, "cover_letter_pdf", "processing")
                        await publish_resume_event({"type": "resume_file_processing", "user_id": user_id, "valid_job_id": valid_job_id, "file_type": "cover_letter_pdf"})
                        ...
                        await publish_resume_event({"type": "resume_file_ready", "user_id": user_id, "valid_job_id": valid_job_id, "file_type": "cover_letter_pdf"})
                    except Exception as e:
                        ...
                        await publish_resume_event({"type": "resume_file_failed", "user_id": user_id, "valid_job_id": valid_job_id, "file_type": "cover_letter_pdf", "error": str(e)})
            else:
            ...
            logger.info("resume_build_completed", valid_job_id=valid_job_id, files=list(results.keys()))
            return results

    except Exception as e:
        logger.exception("resume_build_failed", valid_job_id=valid_job_id, error=str(e))
