...
def test_duplicates_tab_includes_strict_similarity_same_url_and_location_unknown():
    ...
    assert matches_invalid_job_category(LOCATION_UNKNOWN_EXCLUSION, "duplicates") is True
    assert matches_invalid_job_category(None, "duplicates") is True


def test_duplicates_tab_excludes_low_score_extraction_failed_and_non_us():
    ...
    assert matches_invalid_job_category(NON_US_LOCATION_EXCLUSION, "duplicates") is False
